...
class UserTask extends  \app\agent\controller\Base
{
    ...
    public function index(){
        if (Request::isAjax()) {
            ...
            if(!empty($param['range'])){
                ...
                $where[] = ['t.month_end', '=', strtotime($dates[1]."23:59:59")];
            } else {
                $date = time();
                if(date('d') >=10){
                    // echo "if";
            ...
            $list = Db::name('api_task')->alias('t')->join('cloud_times_api_user u', 't.user_id = u.id')->field('u.mobile,u.parent_user_id,u.invitees,u.price,u.level,t.*')->where($where)->where(function($query){
    ...
    public function distribute(){
        ...
        try {
            ...
            Db::name('api_user')->where('id', $user_id)->dec('task_price', $user['task_price'])->inc('price', $user['task_price'])->update();
            if($res){
                Db::commit();
                return 1;
            } else {
