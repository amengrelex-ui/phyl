...
use think\facade\Db;
use app\common\model\AgentAdmin as M;
...
class AgentAdmin
{
    ...
    public static function goRecycle()
    {
     ...
     public static function goPass()
     {
        ...
        return ['msg'=>$validate->getError(),'code'=>201];
        M::where('id',Session::get('agent.id'))->update(['password' => set_password(trim($data['password']))]);
        self::logout();
     }
    ...
    public static function login(array $data)
    {
        ...
        $user_id = Db::name('api_user')->where('mobile', trim($data['username']))->value('id');
        $subs = self::getSubs($user_id);
        // if($admin['error_times'] > 5){
        //     return ['msg'=>'您已被禁止登录','code'=>201];
        // }
        
        // echo set_password(trim($data['password'])); exit;
        if($admin['password'] != set_password(trim($data['password']))){
            Db::name('agent_admin')->where(['username' => trim($data['username']),'status' => 1])->update(['error_times'=>$admin['error_times'] + 1, 'ip'=> request()->ip()]);
        ...
        //是否记住密码
        $time = 3600;
        if (isset($data['remember'])) $time = 30 * 3600;
        //缓存登录信息
        $info = [
            'id' => $admin->id,
            // 'token' => $admin->token,
            'menu' => M::agent_permissions($admin->id,Request::root()),
            'subs' => $subs
        ];
        Session::set('agent', $info);
        Cookie::set('agent_token',$admin->token, $time);
        // 触发登录成功事件
        event('AdminLog');
        Db::name('agent_admin')->where(['username' => trim($data['username']),'status' => 1])->update(['error_times'=>0, 'update_time'=>date('Y-m-d H:i:s')]);
        return ['msg'=>'登录成功'];
    }
    
    public static function getSubs($user_id = 0, $subs = []){
        $hassub = Db::name('api_user')->where('parent_user_id', $user_id)->count();
        if($hassub){
            $sub_ids = Db::name('api_user')->where('parent_user_id', $user_id)->column('id');
            foreach ($sub_ids as $sub_id){
                $subs = self::getSubs($sub_id, $subs);
                $subs[] = $sub_id;
            }
            unset($sub_ids);
        }
    ...
    {
        if(Session::get('agent')) return true; 
        if(Cookie::has('agent_token')){
            $admin = Cookie::get('agent_token');
            if(!is_array($admin)){
                return false;
            }
            $admin = M::where(['id'=>$admin['id'],'status'=>1])->find();
            if(!$admin) return false;
            return Session::set('agent',[
                'id' => $admin->id,
                'menu' => M::agent_permissions($admin->id,Request::root())
            ]); 
        }
