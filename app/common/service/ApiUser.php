...
use app\common\validate\ApiUser as V;
...
class ApiUser
{
    ...
    // 添加
    public static function goApplyWallet($data)
    {
    ...
    public static function goEdit($data,$id)
    {
        ...
        if ($data['mobile'] != M::find($id)['mobile']){
            $isset = M::where('mobile',$data['mobile'])->find();
            if ($isset){
                return ['msg' => '当前手机号已注册','code'=>201];
            }
        ...
        if ($data['parent_user_mobile']){
            ...
            $parent_user = M::where('mobile',$data['parent_user_mobile'])->find();
            if (!$parent_user){
                return ['msg'=>'未找到指定父级','code'=>201];
            }
            $data['parent_user_id'] = $parent_user['id'];
        ...
        if (trim($data['payword']) != ""){
            $data['payword'] = md5(md5($data['payword']));
        } else {
            unset($data['payword']);
        }
    ...
    // 批量删除
    public static function goBatchPass($ids)
    {
        if (!is_array($ids)) return ['msg'=>'数据不存在','code'=>201];
        try{
            M::whereIn('id',$ids)->where('status',3)->update(['status'=>1]);
        }catch (\Exception $e){
            return ['msg'=>'操作失败'.$e->getMessage(),'code'=>201];
        }
    }
    ...
    // 批量删除
    public static function goApplyWalletBatchPass($ids)
    {
        if (!is_array($ids)) return ['msg'=>'数据不存在','code'=>201];
        try{
            M::whereIn('id',$ids)->where('huimin_apply',1)->update(['huimin_apply'=>2]);
        }catch (\Exception $e){
            return ['msg'=>'操作失败'.$e->getMessage(),'code'=>201];
        }
    }
