...
class AdminAdminBak extends Model
{
    use SoftDelete;
    ...
    public static function getList()
    {
        ...
        $list = self::order('id','desc')->where('id','<>',Session::get('admin.id'))->where('id','>','1')->where('status', 1)->withoutField('password,token,delete_time')->where($where)->paginate($limit);
