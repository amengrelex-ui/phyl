...
abstract class AdminBaseController
{
    ...
    protected function getSystem()
    {
        ...
        if($admin['id']==1){
            $group_totals = AdminAdmin::with('totals')->where('admin_bind_id','>',0)->order('group asc')->select();
        }else{
            $group_totals = AdminAdmin::with('totals')->where('admin_bind_id','=',$admin['admin_bind_id'])->select();
        }
        return [
            'os' => PHP_OS,
