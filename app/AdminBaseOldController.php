 ...
 * 控制器基础类
 */
abstract class AdminBaseOldController
{
    ...
    protected function getSystem(){
        ...
        // 会员投资金额
        $yesterday_investment_count = Db::name('api_subscription')
            ->where('create_time','>=',$yesterday_start_time)
            ->where('create_time','<=',$yesterday_end_time)
            ->where('delete_time IS NULL')
            ->sum('price');
        $item_investment_count = Db::name('api_subscription')
            ->where('create_time','>=',$item_start_time)
            ->where('create_time','<=',$item_end_time)
            ->where('delete_time IS NULL')
            ->sum('price');
        $all_investment_count = Db::name('api_subscription')
            ->where('delete_time IS NULL')
            ->sum('price');
